...
async def _run_acceptold(admin_id: int, status_msg, specific_chat_id=None):
    ...
    try:
        ...
        for cid in chats_to_process:
            try:
                # PeerChannel directly build karo — entity cache ki zaroorat nahi
                from telethon.tl.types import PeerChannel, PeerChat, InputPeerChannel
                raw_id = abs(cid)
                id_str = str(raw_id)
                # -100XXXXXXXXX format se actual channel id nikalo
        ...
        for cid in chats_to_process:
            try:
                ...
                if not entity:
                    try:
                        await client(HideAllChatJoinRequestsRequest(
                            peer=PeerChannel(channel_id), approved=True
                        ))
                        accepted += 1
                        continue
                    except Exception as e3:
