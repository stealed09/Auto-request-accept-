...
def build_welcome_keyboard() -> InlineKeyboardMarkup | None:
    rows = []
    # Custom buttons (admin ne add kiye)
    ...
    # Report + Episodes buttons — hamesha niche dikhenge
    report_link   = get_setting("report_link", "")
    episodes_link = get_setting("episodes_link", "")
    bottom_row = []
    if report_link:
        bottom_row.append(InlineKeyboardButton(text="🚨 Report Issue", url=report_link))
    if episodes_link:
        bottom_row.append(InlineKeyboardButton(text="🎬 Latest Episodes", url=episodes_link))
    if bottom_row:
    ...
    if channels:
        ...
        for chat_id, title, accept in channels:
            ...
            buttons.append([
                ...
                InlineKeyboardButton(text="⏮️", callback_data=f"chat_acceptold:{chat_id}"),
