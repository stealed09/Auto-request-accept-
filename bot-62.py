...
def build_chats_keyboard(page: int = 0):
    cur.execute("SELECT chat_id, title, accept FROM chats ORDER BY title")
    rows = cur.fetchall()
    PER_PAGE = 6
    total = len(rows)
    start = page * PER_PAGE
    ...
    buttons = []
    for chat_id, title, accept in chunk:
        name = (title or str(chat_id))[:28]
        status = "🟢" if accept else "🔴"
        buttons.append([
            InlineKeyboardButton(
                text=f"{status} {name}",
                callback_data=f"chtoggle:{chat_id}:{page}"
            )
        ])
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="◀️ Prev", callback_data=f"chpage:{page-1}"))
    if start + PER_PAGE < total:
        nav.append(InlineKeyboardButton(text="Next ▶️", callback_data=f"chpage:{page+1}"))
    if nav:
        buttons.append(nav)
...
async def cmd_chats(msg: Message):
    if not is_admin(msg.from_user.id):
        return await msg.reply("⚠️ Only admins can use this.")
    kb, total = build_chats_keyboard(0)
    await msg.reply(
        f"📋 <b>Chat List</b> — <i>{total} chats</i>\n"
...
async def cb_chtoggle(cb: CallbackQuery):
    ...
    chat_id = int(chat_id)
    page = int(page)
    cur.execute("SELECT accept FROM chats WHERE chat_id=?", (chat_id,))
    row = cur.fetchone()
    ...
    await cb.answer(f"Auto-accept: {status}", show_alert=False)
    kb, total = build_chats_keyboard(page)
...
async def cb_chpage(cb: CallbackQuery):
    ...
    kb, _ = build_chats_keyboard(page)
    await cb.message.edit_reply_markup(reply_markup=kb)
...
async def cmd_help(msg: Message):
    await msg.reply(
        ...
        "/chats — Per-chat auto-accept toggle (buttons)\n"
