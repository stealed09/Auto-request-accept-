...
async def cb_cmd(cb: CallbackQuery, state: FSMContext):
    ...
    if cmd == "autoaccept_on":
        ...
        await cb.message.edit_text(
            "🟢 <b>Auto-Accept ON!</b>\nSab join requests automatically accept honge.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Back", callback_data="menu:setup")]
            ])
        )
        return
    ...
    if cmd == "autoaccept_off":
        ...
        await cb.message.edit_text(
            "🔴 <b>Auto-Accept OFF!</b>\nJoin requests manually accept karni padegi.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Back", callback_data="menu:setup")]
            ])
    ...
    if cmd == "acceptold":
        await cb.message.answer("⏳ <b>Purane requests accept ho rahe hain...</b>", parse_mode="HTML")
        await _do_acceptold(cb.message.chat.id, None)
        return
    ...
    if cmd == "login":
        await cb.message.answer(
            "🔐 <b>Login shuru karo:</b>\n\n"
            "<code>/login</code> command use karo.\n"
            "my.telegram.org se API ID aur API Hash chahiye.",
            parse_mode="HTML"
        )
        return
    ...
    if cmd == "logout":
        await cb.message.answer(
            "🚪 <b>Logout karne ke liye:</b>\n\n"
            "<code>/logout</code> command use karo.",
            parse_mode="HTML"
        )
        return
...
async def flow_addchannel(msg: Message, state: FSMContext):
    ...
    await _do_add_chat(msg, state, "channel")
...
async def flow_addgroup(msg: Message, state: FSMContext):
    ...
    if msg.text and msg.text.startswith("/skip"):
        await state.clear()
        return await msg.reply("❌ Cancelled.")
    await _do_add_chat(msg, state, "supergroup")
...
async def flow_removechat(msg: Message, state: FSMContext):
    ...
    if deleted:
        await msg.reply(f"🗑 <b>{title or chat_id}</b> removed.", parse_mode="HTML")
    else:
        await msg.reply(f"⚠️ Ye chat registered nahi tha.")
...
async def flow_addadmin(msg: Message, state: FSMContext):
    ...
    try:
        ...
        return await msg.reply("❌ Valid User ID do (numbers only).")
    cur.execute("INSERT OR IGNORE INTO admins VALUES (?)", (uid,))
    conn.commit()
    await msg.reply(f"✅ <code>{uid}</code> admin ban gaya.", parse_mode="HTML")
...
async def flow_removeadmin(msg: Message, state: FSMContext):
    ...
    conn.commit()
    if cur.rowcount:
        await msg.reply(f"🗑 <code>{uid}</code> admin list se remove.", parse_mode="HTML")
    else:
        await msg.reply(f"⚠️ <code>{uid}</code> admin nahi tha.", parse_mode="HTML")
...
async def flow_blacklist_add(msg: Message, state: FSMContext):
    ...
    conn.commit()
    await msg.reply(f"🚫 <code>{uid}</code> blacklisted.", parse_mode="HTML")
...
async def flow_blacklist_remove(msg: Message, state: FSMContext):
    ...
    conn.commit()
    if cur.rowcount:
        await msg.reply(f"✅ <code>{uid}</code> blacklist se remove.", parse_mode="HTML")
    else:
        await msg.reply(f"⚠️ <code>{uid}</code> blacklist mein tha hi nahi.", parse_mode="HTML")
...
async def cmd_help(msg: Message):
    ...
    commands_text = (
        ...
        f"├ /pin — Reply → message pin karo\n"
        f"├ /unpin — Message unpin karo\n"
