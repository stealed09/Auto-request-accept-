...
async def _run_acceptold(admin_id: int, status_msg, specific_chat_id=None):
    ...
    try:
        ...
        for cid in chats_to_process:
            try:
                async for req in client.get_chat_join_requests(cid):
                    try:
                        await client.approve_chat_join_request(cid, req.user.id)
                        accepted += 1
                ...
                async for req in client.get_chat_join_requests(cid):
                    try:
                        if approve:
                            ...
                            await client.decline_chat_join_request(cid, req.user.id)
                        ok += 1
                        await asyncio.sleep(0.3)
                    except Exception:
                        fail += 1
