    ...
    try:
        ...
        for cid in chats_to_process:
            try:
                ...
                # resolve_peer — reference code style
                try:
                    ...
                    raw_id = abs(cid)
                    id_str = str(raw_id)
                    ch_id = int(id_str[3:]) if id_str.startswith("100") and len(id_str) > 10 else raw_id
                    peer = _IC(channel_id=ch_id, access_hash=0)
                ...
                for importer in getattr(reqs, "importers", []):
                    try:
                        user_peer = await client.resolve_peer(importer.user_id)
        ...
        for cid in chats_to_process:
            try:
                ...
                reqs = await client(GetChatInviteImportersRequest(
                    ...
                    offset_date=0, offset_user=InputUserEmpty(), limit=100
                ))
                for importer in getattr(reqs, "importers", []):
                    try:
                        user_peer = await client.resolve_peer(importer.user_id)
                        await client(HideChatJoinRequestRequest(
                            peer=peer, user_id=user_peer, approved=True
                        ))
                        accepted += 1
                    except Exception as e2:
                        log.warning(f"approve failed: {e2}")
