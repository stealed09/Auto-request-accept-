...
# Telethon version compatibility
try:
    # Telethon 1.44+ — GetChatInviteImportersRequest with requested=True fetches pending join requests
    from telethon.tl.functions.messages import (
        GetChatInviteImportersRequest,
        HideChatJoinRequestRequest,
        HideAllChatJoinRequestsRequest
    )
    _telethon_has_invite_reqs = True
except ImportError:
    try:
        # Older telethon
        from telethon.tl.functions.messages import GetChatInviteRequestsRequest as GetChatInviteImportersRequest
        from telethon.tl.functions.messages import HideChatJoinRequestRequest, HideAllChatJoinRequestsRequest
        _telethon_has_invite_reqs = True
    except ImportError:
        GetChatInviteImportersRequest = None
        HideChatJoinRequestRequest    = None
        HideAllChatJoinRequestsRequest = None
        _telethon_has_invite_reqs = False
...
async def _run_acceptold(admin_id: int, status_msg, specific_chat_id=None):
    ...
    try:
        ...
        failed   = 0
        err_detail = ""
        # Ek baar dialogs fetch karo — sab entities cache ho jayengi
        try:
            ...
            for d in dialogs:
                if d.entity:
                    ...
                    if eid:
                        dialog_map[eid] = d.entity
                        dialog_map[int(f"-100{eid}")] = d.entity
        except Exception as de:
            log.warning(f"get_dialogs failed: {de}")
            dialog_map = {}
        ...
        for cid in chats_to_process:
            try:
                ...
                if id_str.startswith("100") and len(id_str) > 10:
                    channel_id = int(id_str[3:])
                else:
                    channel_id = raw_id

                log.warning(f"DEBUG acceptold: cid={cid} raw_id={raw_id} channel_id={channel_id}")

                entity = dialog_map.get(cid)
                log.warning(f"DEBUG entity from dialog_map: {entity}")

                if not entity:
                    try:
                        entity = await client.get_entity(cid)
                        log.warning(f"DEBUG get_entity success: {entity}")
                    except Exception as ge:
                        log.warning(f"DEBUG get_entity failed: {ge}")
                        pass

                # Try 3: HideAllChatJoinRequestsRequest — entity ke bina bhi kaam karta hai
                if not entity:
                    try:
                        ...
                        err_detail = f"Chat {cid}: {e3}"
                        failed += 1
                        continue

                from telethon.tl.types import InputUser
                offset_date = None
                offset_user = InputUser(0, 0)
                while True:
                    reqs = await client(GetChatInviteImportersRequest(
                        peer=entity,
                        requested=True,
                    ...
                    if not reqs.importers:
                        break
                    for importer in reqs.importers:
                        try:
                            await client(HideChatJoinRequestRequest(
                                peer=entity,
                                user_id=__import__("telethon.tl.types", fromlist=["InputUser"]).InputUser(importer.user_id, 0),
                                approved=True
                            ))
                            accepted += 1
                            ...
                            err_detail = str(e2)
                            failed += 1
                    if len(reqs.importers) < 100:
                        break
                    offset_date = reqs.importers[-1].date
                    from telethon.tl.types import InputUser as _IU
                    offset_user = _IU(reqs.importers[-1].user_id, 0)
...
async def _fetch_and_show_chats(user_id: int, chat_id: int, status_msg_id: int, page: int = 0):
    ...
    try:
        ...
        all_chats = []  # list of (cid, title, username, type)
        for d in dialogs:
            if not d.entity:
                continue
            ent = d.entity
            # Sirf wahi dikhao jahan account admin ya creator hai
            ...
            if isinstance(ent, Channel):
                ...
                all_chats.append((cid, ent.title or str(cid), uname, ctype))
            elif isinstance(ent, Chat):
                cid   = -ent.id
                uname = None
                all_chats.append((cid, ent.title or str(cid), uname, "group"))
        await client.disconnect()

        # Already added chat IDs
        ...
        for cid, title, uname, ctype in slice_:
            ...
            already = "✅" if cid in added_ids else ""
            short   = title[:28]
            # Name row
            lines.append(f"\n{emoji} <b>{title}</b>")
            if uname:
            ...
            # Button row — Add ya Already Added
            if cid in added_ids:
                buttons.append([InlineKeyboardButton(
                    text=f"✅ {short[:22]} (added)",
                    callback_data="noop"
                )])
            else:
                buttons.append([InlineKeyboardButton(
                    text=f"➕ {emoji} {short[:22]}",
                    callback_data=f"fetchadd:{cid}:{ctype}:{title[:30]}"
                )])
...
async def _mass_accept_reject(user_id: int, chat_id: int, status_msg_id: int, approve: bool, filter_type: str = "all"):
    ...
    try:
        ...
        for ent in targets:
            ok = fail = 0
            try:
                if not _telethon_has_invite_reqs:
                    details.append(f"⚠️ <b>{ent.title[:22]}</b>: Telethon version mein support nahi")
                    continue
                reqs = await client(GetChatInviteImportersRequest(peer=ent, requested=True, offset_date=None, offset_user=__import__('telethon.tl.types', fromlist=['InputUser']).InputUser(0,0), limit=100))
                if not reqs.importers:
                    continue
                for req in reqs.importers:
                    try:
                        await client(HideChatJoinRequestRequest(peer=ent, user_id=req.user, approved=approve))
                        ok += 1
    ...
    try:
        ...
        try:
            ...
            for d in dialogs:
                if d.entity:
                    ...
                    if eid:
                        dialog_map[eid] = d.entity
                        # -100 prefix wala ID bhi map karo
                        dialog_map[int(f"-100{eid}")] = d.entity
        except Exception as de:
            log.warning(f"get_dialogs failed: {de}")
            dialog_map = {}
        ...
        for cid in chats_to_process:
            try:
                ...
                raw_id = abs(cid)
                id_str = str(raw_id)
                channel_id = int(id_str[3:]) if (id_str.startswith("100") and len(id_str) > 10) else raw_id
                ...
                if not entity:
                    try:
                        from telethon.tl.types import InputPeerChannel
                        await client(HideAllChatJoinRequestsRequest(
                            peer=InputPeerChannel(channel_id=channel_id, access_hash=0), approved=True
                        ))
                        accepted += 1
                        continue
                    except Exception as e3:
                ...
                while True:
                    reqs = await client(GetChatInviteImportersRequest(
                        peer=entity, requested=True,
                        offset_date=offset_date, offset_user=offset_user, limit=100
                    ))
                    if not reqs.importers:
                        break
                    for importer in reqs.importers:
                        try:
                            await client(HideChatJoinRequestRequest(
                                peer=entity, user_id=__import__("telethon.tl.types", fromlist=["InputUser"]).InputUser(importer.user_id, 0), approved=True
                            ))
                            accepted += 1
                        except Exception as e2:
                            log.warning(f"approve failed: {e2}")
                    ...
                    log.warning("GetChatInviteImportersRequest not available")
            except Exception as e:
        ...
        await bot.send_message(
            ...
            f"Accepted: <b>{accepted}</b>\n"
            f"Failed: <b>{failed}</b>"
            + (f"\n\n<b>Last error:</b>\n<code>{err_detail}</code>" if failed and 'err_detail' in dir() else ""),
            parse_mode="HTML"
        )
    finally:
        await client.disconnect()
...
async def _acceptold_task(
    ...
    try:
        ...
        for chat_id in chat_ids:
            ...
            fail = 0
            try:
                entity = dmap.get(chat_id)
                if not entity:
                    entity = await client.get_entity(chat_id)
                # Pending join requests fetch karo
                offset_date = None
                offset_user = None
                while True:
                    reqs = await client(GetChatInviteImportersRequest(
