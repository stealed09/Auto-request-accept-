...
async def cb_cmd(cb: CallbackQuery, state: FSMContext):
    ...
    if cmd == "autoaccept_on":
        ...
        await cb.answer("🟢 Auto-accept ON!", show_alert=True)
        return
    if cmd == "autoaccept_off":
        set_setting("accept_mode", "manual")
        await cb.answer("🔴 Auto-accept OFF!", show_alert=True)
        return
    if cmd == "clearbuttons":
        set_setting("welcome_buttons", "")
        await cb.answer("🗑 Buttons clear ho gaye!", show_alert=True)
        return
    if cmd == "stats":
        ...
        st = "🟢 ON" if mode == "auto" else "🔴 OFF"
        await cb.answer(
            f"📊 Chats: {total_chats} | Users: {total_users}\nToday: {today_joins} | Total: {total_joins}\nAuto-Accept: {st}",
            show_alert=True
        )
        return
    if cmd == "ping":
        await cb.answer(f"🏓 PONG! Uptime: {uptime_str()}", show_alert=True)
        return

    # Show hint for other commands
    hint = CMD_HINTS.get(cmd, f"<code>/{cmd}</code> — yeh command use karo.")
    await cb.message.answer(hint, parse_mode="HTML")
    await cb.answer()
