        ...
        for cid in chats_to_process:
            try:
                ...
                while True:
                    ...
                    last = reqs.importers[-1].user
                    from telethon.tl.types import InputUser as _IU
                    offset_user = _IU(last.id, last.access_hash) if hasattr(last, 'access_hash') else _IU(last.id, 0)
            ...
            try:
                ...
                while True:
                    ...
                    for importer in reqs.importers:
                        try:
                            await client(HideChatJoinRequestRequest(
                                peer=entity, user_id=importer.user, approved=True
                            ))
                            accepted += 1
                        except Exception as e2:
                            log.warning(f"approve failed: {e2}")
