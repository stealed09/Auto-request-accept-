...
async def _run_acceptold(admin_id: int, status_msg, specific_chat_id=None):
    ...
    try:
        ...
        if not await client.is_user_authorized():
            ...
            return
        _use_get_requests = _telethon_has_invite_reqs
        if specific_chat_id:
        ...
        for cid in chats_to_process:
            try:
                ...
                if _use_get_requests:
                    reqs = await client(GetChatInviteImportersRequest(peer=entity, requested=True, offset_date=None, offset_user=None, limit=100))
                    for req in reqs.importers:
                        try:
                            await client(HideChatJoinRequestRequest(peer=entity, user_id=req.user, approved=True))
                            accepted += 1
                        except Exception:
                            pass
                else:
                    log.warning(f"GetChatInviteImportersRequest not available")
            except Exception as e:
...
async def on_join_request(req: ChatJoinRequest):
    ...
    try:
        ...
        # ── Approval DM — image + styled text ──────────────────
        approval_img     = get_setting("approval_image", "")
        approval_link    = get_setting("approval_link", "")
        approval_caption = (
            f"• REQUEST TO JOIN •\n\n"
            f"HEY {first_name},\n\n"
            f"YOUR REQUEST TO JOIN {title} HAS BEEN APPROVED.\n\n"
            f"• JOIN OUR NETWORK •\n"
            f"• JOIN {title} •"
        )
        btns = []
        if approval_link:
            btns.append([InlineKeyboardButton(text=f"JOIN {title}", url=approval_link)])
        ...
        try:
            if approval_img:
                ...
                await bot.send_message(
                    user_id,
                    f"<b>• REQUEST TO JOIN •</b>\n\n"
                    f"HEY <b>{first_name}</b>,\n\n"
                    f"<blockquote>YOUR REQUEST TO JOIN <b>{title}</b> HAS BEEN APPROVED.</blockquote>\n\n"
                    f"<b>• JOIN OUR NETWORK •</b>\n"
                    f"<b>• JOIN {title} •</b>",
                    parse_mode="HTML",
                    reply_markup=approval_kb
                )
...
async def _do_broadcast(source_chat_id: int, source_msg_id: int, mode: str, status_msg: Message):
    ...
    cur.execute("UPDATE broadcast_log SET total_sent=? WHERE id=?", (ok, broadcast_id))
    conn.commit()
    ...
    await status_msg.edit_text(
        ...
        f"<b>Broadcast ID: <code>{broadcast_id}</code></b>\n"
        f"Delete karne ke liye:\n"
        f"<code>/dBroadcast {broadcast_id}</code>",
        parse_mode="HTML",
...
async def cmd_delete_broadcast(msg: Message):
    ...
    cur.execute("DELETE FROM broadcast_log WHERE id=?", (broadcast_id,))
    conn.commit()
