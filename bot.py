...
def parse_button_text(text: str):
    ...
    Syntax:
      Normal | https://link        → queued for 2-per-row
      ^Full Width^ | https://link  → always alone on its row
    """
    ...
    for line in lines:
        ...
        if raw_name.startswith("^") and raw_name.endswith("^"):
            name = raw_name[1:-1].strip()
            entries.append((name, link, True))
        else:
            entries.append((raw_name, link, False))
...
async def cmd_help(msg: Message):
    await msg.reply(
        ...
        "/autoaccept on|off — Join request toggle\n"
