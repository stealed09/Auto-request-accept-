...
async def _flow_add_chat_common(msg: Message, state: FSMContext, forced_type: str = None):
    """Channel aur Group dono ke liye shared handler — auto-detect karta hai type."""
    ...
    # Auto-detect: Bot API se actual type lo
    if ctype == "channel":
        actual_type = "channel"
    elif ctype in ("group", "supergroup"):
        actual_type = ctype
    elif ctype == "unknown" and forced_type:
        actual_type = forced_type
    else:
        # Last resort — ID se guess karo, default channel
    ...
    if not admin["is_admin"]:
        return await status.edit_text(
            f"❌ Bot <b>{title}</b> mein admin nahi hai!\n<i>Bot ko admin banao phir dobara try karo.</i>",
            parse_mode="HTML", reply_markup=BACK_KB("setup")
        )
    if not admin["can_invite"]:
        return await status.edit_text(
    ...
    await status.edit_text(
        ...
        f"━━━━━━━━━━▧▣▧━━━━━━━━━━\n"
        f"➺ <b>Name:</b> {title}\n"
        f"➺ <b>Type:</b> {type_emoji} {actual_type}\n"
        f"➺ <b>Username:</b> {uname}\n"
        f"➺ <b>ID:</b> <code>{chat_id}</code>\n"
        f"➺ <b>Auto-Accept:</b> 🟢 ON",
        parse_mode="HTML", reply_markup=BACK_KB("setup")
