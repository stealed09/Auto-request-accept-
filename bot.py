...
def build_setup_buttons():
    ...
    rows = cur.fetchall()
    buttons = []
    # Per-chat rows: toggle + acceptold
    for chat_id, title, chat_type, accept in rows:
        emoji = "📢" if chat_type == "channel" else "👥"
        st    = "🟢" if accept else "🔴"
        short = (title or str(chat_id))[:18]
        buttons.append([
            InlineKeyboardButton(text=f"{emoji} {st} {short}", callback_data=f"chat_toggle:{chat_id}"),
            InlineKeyboardButton(text="⏮️", callback_data=f"chat_acceptold:{chat_id}"),
            InlineKeyboardButton(text="🗑", callback_data=f"chat_remove:{chat_id}"),
        ])
    ...
    buttons += [
        ...
        [InlineKeyboardButton(text="📝 Log Channel", callback_data="cmd:setlog")],
