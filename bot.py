...
async def _run_acceptold(admin_id: int, status_msg, specific_chat_id=None):
    ...
    try:
        ...
        for cid in chats_to_process:
            try:
                # Step 1: Peer cache warm karo
                try:
                    await client.get_dialogs(limit=5)
                except Exception:
                    pass

                # Step 2: Entity resolve karo with fallback
                try:
                    entity = await client.get_entity(cid)
                except Exception:
                    # Manual InputChannel fallback (reference code style)
                    from telethon.tl.types import InputChannel
                    raw_id = abs(cid)
                    id_str = str(raw_id)
                    channel_id = int(id_str[3:]) if id_str.startswith("100") else raw_id
                    entity = InputChannel(channel_id=channel_id, access_hash=0)
                    log.warning(f"Using manual InputChannel for {cid}")
        ...
        for cid in chats_to_process:
            try:
                ...
                try:
                    ...
                    pass
                # Entity resolve with InputChannel fallback
                try:
                    entity = await client.get_entity(cid)
                except Exception:
                    from telethon.tl.types import InputChannel
